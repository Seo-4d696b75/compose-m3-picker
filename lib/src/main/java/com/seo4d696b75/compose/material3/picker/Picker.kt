...
private fun <T> PickerLabels(
    ...
    LazyLayout(
        ...
        val labels = state.onLayout(labelHeight, dividerHeightPx)

        val labelConstraints = Constraints.fixed(
            width = width,
            height = labelHeight
        )
        val placeableMap = labels.associate { label ->
            label.rawIndex to
                    compose(label.compositionIndex).first().measure(labelConstraints)
        }
        ...
        layout(width, height) {
            placeableMap.forEach { (rawIndex, placeable) ->
                placeable.placeWithLayer(
                    ...
                    val distance = (rawIndex - state.rawIndex).absoluteValue.coerceAtMost(1f)
